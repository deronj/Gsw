//
//  AirPlaneObject.swift
//  Gsw
//
//  Created by Deron Johnson on 7/31/17.
//  Copyright © 2017 Deron Johnson. All rights reserved.
//

import MetalKit

class AirplaneObject: MDLMeshObject
{
    // For Debug: to disable texture, set this to false
    // TODO: force this to false until we implement a lit texture shader
    let textured = false
    
    // TODO: currently, the resource must contain only one mesh
    let _resourceName = "realship.obj"
    ...
    public init(transform: TRSTransform? = nil, device: MTLDevice) throws
    {
        ...
        try super.init(mdlMesh:mdlMeshes![0] as! MDLMesh, mtkMesh:mtkMeshes[0], transform:transform, device:device,
                       defaultMaterials:!textured)
    }
